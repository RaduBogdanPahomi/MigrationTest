...
protocol MoviesServiceable {
    func getMovieList(page: Int, action: String) async -> Result<MovieList, RequestError>
...
struct MovieService: HTTPClient, MoviesServiceable {
    func getMovieList(page: Int, action: String) async -> Result<MovieList, RequestError> {
        return await sendRequest(endpoint: MoviesEndpoint.movieList(page: page, action: action), responseModel: MovieList.self)
    }
